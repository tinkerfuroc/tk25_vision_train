...
class RealSenseDatasetCreator:
    ...
    def _process_captured_frame(self, cv_image, mask_annotator, label_annotator, padding):
        """Process a single captured frame through the annotation workflow."""
        print("Frame captured. Running segmentation...")
        
        # Run automatic segmentation
        predictions, rgb_image = self._run_automatic_segmentation(cv_image)
        ...
        # Interaction loop for the captured frame
        while True:
            # Add padding to the base image
            display_image = cv2.copyMakeBorder(cv_image, top, bottom, left, right, 
    ...
    def run(self):
        ...
        try:
            while True:  # Main loop for live feed and capturing
                ...
                cv2.putText(display_image, "Press SPACE to capture, 'q' to quit", (10, 30), 
                ...
                result = self._process_captured_frame(cv_image, mask_annotator, label_annotator, padding)
