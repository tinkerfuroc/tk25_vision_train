...
from yolo_tuning.vision_tuning.training.specs import TrainJob


DETECTION_JOB = TrainJob(task="det", project_name="yolo_finetuned", default_epochs=50, default_batch=8)
SEGMENTATION_JOB = TrainJob(task="seg", project_name="yolo_seg_finetuned", default_epochs=250, default_batch=4)
...
def _plot_training_results(results_dir: str, task: str) -> None:
    ...
    if task == "seg":
        ...
        maps = [("metrics/mAP50-95(M)", "metrics/mAP50(M)", "Mean Average Precision (Masks)")]
    else:
        metrics = [
            ("train/box_loss", "val/box_loss", "Bounding Box Loss"),
            ("train/cls_loss", "val/cls_loss", "Classification Loss"),
        ...
        maps = [("metrics/mAP50-95(B)", "metrics/mAP50(B)", "Mean Average Precision (Boxes)")]
...
def run_training_job(
    *,
    ...
    _plot_training_results(results_dir, task=job.task)

    best_model_path = os.path.join(results_dir, "weights", "best.pt")
    exported_best = ""
    if os.path.exists(best_model_path):
        exported_best = os.path.join(os.getcwd(), f"{job.project_name}_best.pt")
...
def run_detector_training(
    ...
    *,
    ...
    epochs: int = DETECTION_JOB.default_epochs,
    batch: int = DETECTION_JOB.default_batch,
    imgsz: int = DETECTION_JOB.default_imgsz,
...
def run_segmentation_training(
    ...
    *,
    dataset_path: Optional[str] = None,
    epochs: int = SEGMENTATION_JOB.default_epochs,
    batch: int = SEGMENTATION_JOB.default_batch,
    imgsz: int = SEGMENTATION_JOB.default_imgsz,
    train_ratio: float = 0.8,
) -> Tuple[str, str]:
