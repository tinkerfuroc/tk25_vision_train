...
class SAM3SegmentationCollector:
    ...
    def _ensure_segmenter(self):
        ...
        except Exception as exc:  # pragma: no cover - real SAM3 not available in tests
            raise RuntimeError("SAM3 is not installed. Provide a segmenter or install SAM3.") from exc
        self.segmenter = LanguageGroundedSAM3()
        return self.segmenter

    def _iter_masks(self, detections: Iterable) -> List[SAM3Mask]:
    ...
    def _write_label_file(self, mask: np.ndarray, class_id: int, h: int, w: int, fh) -> None:
    ...
    def _save_frame(self, frame: np.ndarray, detections: Sequence[SAM3Mask], idx: int) -> bool:
        ...
        with open(label_path, "w") as fh:
            for det in detections:
                ...
                self._write_label_file(det.mask, class_id, h, w, fh)
                saved = True
        return saved
    ...
    def collect(self, frames: Sequence[np.ndarray], prompts: Optional[Sequence[str]] = None) -> int:
        ...
        for idx, frame in enumerate(frames):
            if idx == 0 or previous_masks is None or self.tracker is None:
                detections = segmenter.segment(frame, prompts)  # type: ignore[attr-defined]
            else:
                detections = self.tracker.track(previous_masks, frame)  # type: ignore[attr-defined]
                if not detections:
                    detections = segmenter.segment(frame, prompts)  # type: ignore[attr-defined]
